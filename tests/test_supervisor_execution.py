"""Tests for RedThreadSupervisor.invoke() end-to-end orchestration and runtime review."""
...
async def test_supervisor_invoke_dry_run_returns_campaign_result() -> None:
    ...
    with (
        ...
        patch(
            ...
            new=AsyncMock(
                side_effect=[
                    {"result_dict": r.model_dump(mode="json"), "error": None} for r in mock_results
                ]
            ),
        ),
    ):
...
async def test_supervisor_invoke_runs_agentic_runtime_review_for_tool_agent_surface() -> None:
    ...
    with (
        ...
        patch(
            "redthread.orchestration.graphs.attack_graph.run_attack_worker",
            new=AsyncMock(
                return_value={"result_dict": clean_result.model_dump(mode="json"), "error": None}
            ),
        ),
    ):
        supervisor = RedThreadSupervisor(settings)
