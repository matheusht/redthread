...
def test_default_runtime_replay_artifacts_expand_starter_benign_pack() -> None:
    assert DEFAULT_DEFENSE_REPLAY_SUITE_ID == "default-defense-replay-v4"
    ...
    assert any(case.case_id == "yaml_status_note" for case in DEFAULT_BENIGN_REPLAY_CASES)
...
def test_default_replay_suite_uses_runtime_artifacts() -> None:
    ...
    assert len(suite.exploit_cases) == 3
    ...
    assert any(case.case_id == "exploit_roleplay_probe" for case in suite.exploit_cases)
    assert any(case.case_id == "meeting_timezone" for case in suite.benign_cases)
    assert any(case.case_id == "yaml_status_note" for case in suite.benign_cases)
